  ...
  async getAll(loan_id: string) {
    ...
    try {
      let result: any = await entityManager.query(
        ...
        where delete_flag = 'N' and loan_id = $1`, [loan_id],
      );
      return {
        statusCode: 200,
        data: result,
  ...
  async delete(offer_id: string) {
    ...
    try {
      let checkInfo: any = await entityManager.query(
        `select * from tblloanoffers where id = $1`, [offer_id],
      );
      if (checkInfo.length > 0) {
        let result: any = await entityManager.query(
          `update tblloanoffers set delete_flag = 'Y'
          where delete_flag = 'N' and id = $1`, [offer_id],
        );
    ...
    try {
      ...
      if (loanInfo.length > 0) {
        ...
        await entityManager.query(
          `update tblloan set status_flag = 'waiting', step = 5 where id = $1`,
          [loan_id],
        );
